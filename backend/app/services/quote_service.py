"""
Quote service with business logic.
"""
...
from app.schemas.quote import (
    ...
    QuoteWeeklyHoursCreate, QuoteWeeklyHoursResponse,
    AutoFillRequest, AutoFillPattern,
    QuoteTotalsResponse, WeeklyTotal, MonthlyTotal, RoleTotal,
    QuotePhaseResponse,
)
...
class QuoteService(BaseService):
    ...
    def __init__(self, session: AsyncSession):
        ...
        self.line_item_repo = QuoteLineItemRepository(session)
        self.weekly_hours_repo = QuoteWeeklyHoursRepository(session)
    ...
    async def create_quote(self, quote_data: QuoteCreate) -> QuoteResponse:
        ...
        quote = await self.quote_repo.get_with_line_items(quote.id)
        if not quote:
            raise ValueError("Failed to retrieve created quote")
        
        # Log line items for debugging
        logger.info(f"Quote {quote.id} has {len(quote.line_items) if quote.line_items else 0} line items after creation")
        if quote.line_items:
            for li in quote.line_items:
                logger.info(f"  Line item {li.id}: employee_id={li.employee_id}, role_id={li.role_id}")
        
        response = self._to_response(quote, include_line_items=True)
    ...
    async def get_quote(self, quote_id: UUID) -> Optional[QuoteResponse]:
        ...
        if not quote:
            return None
        return self._to_response(quote, include_line_items=False)
    
    async def get_quote_detail(self, quote_id: UUID) -> Optional[QuoteDetailResponse]:
        """Get quote with all line items and weekly hours."""
        quote = await self.quote_repo.get_with_line_items(quote_id)
        if not quote:
            return None
        return self._to_detail_response(quote)
    ...
    async def list_quotes(
        ...
        return [self._to_response(q, include_line_items=False) for q in quotes], total
    ...
    async def clone_quote(self, quote_id: UUID, new_name: str) -> QuoteResponse:
        ...
        new_quote_dict = {
            ...
            "attributes": quote.attributes.copy() if quote.attributes else {},
        }
        new_quote = await self.quote_repo.create(**new_quote_dict)
        
        # Clone phases
        from app.db.repositories.quote_phase_repository import QuotePhaseRepository
        phase_repo = QuotePhaseRepository(self.session)
        if quote.phases:
                ...
                if line_item.weekly_hours:
                    for weekly_hour in line_item.weekly_hours:
                        await self.weekly_hours_repo.create(
                            quote_line_item_id=new_line_item.id,
        ...
        new_quote = await self.quote_repo.get_with_line_items(new_quote.id)
        if not new_quote:
            raise ValueError("Failed to retrieve cloned quote")
        return self._to_detail_response(new_quote)
    ...
    async def create_line_item(
        ...
        if rate == 0 or cost == 0:
            default_rate, default_cost = await self._get_default_rates(
                ...
                line_item_data.currency or quote.currency,
        ...
        # Get max row_order
        max_order = await self.line_item_repo.get_max_row_order(quote_id)
        ...
        line_item_dict["currency"] = line_item_dict.get("currency") or quote.currency
    ...
    async def _sync_employee_with_engagement(self, quote_id: UUID, employee_id: UUID) -> None:
        ...
        # Check if employee is still in any line items for this quote
        line_items = await self.line_item_repo.list_by_quote(quote_id)
        employee_still_in_quote = any(
    ...
    async def delete_line_item(self, quote_id: UUID, line_item_id: UUID) -> bool:
        """Delete a line item."""
        logger.info(f"Attempting to delete line item {line_item_id} from quote {quote_id}")
        ...
        if line_item.quote_id != quote_id:
            logger.warning(f"Line item {line_item_id} belongs to quote {line_item.quote_id}, not {quote_id}")
            return False
    ...
    def _to_response(self, quote: Quote, include_line_items: bool = False) -> QuoteResponse:
        ...
        if include_line_items:
            ...
            try:
                ...
                if line_items_attr and line_items_attr.loaded_value is not None:
                    ...
                    if line_items_list:
                        # Ensure we're iterating over QuoteLineItem objects, not weekly_hours
                        quote_dict["line_items"] = [
                            self._line_item_to_response(li) 
                            for li in line_items_list 
                            if isinstance(li, QuoteLineItem)
                        ]
            except (AttributeError, KeyError, TypeError):
                pass
    ...
    def _to_detail_response(self, quote: Quote) -> QuoteDetailResponse:
        ...
        return QuoteDetailResponse.model_validate(response_dict)
    
    def _line_item_to_response(self, line_item: QuoteLineItem) -> QuoteLineItemResponse:
            ...
            if weekly_hours_attr and weekly_hours_attr.loaded_value is not None:
                ...
                if weekly_hours_list:
                    ...
                    line_item_dict["weekly_hours"] = [
                        ...
                        if isinstance(wh, QuoteWeeklyHours)
                    ]
        except (AttributeError, KeyError, TypeError):
            pass
        ...
        response = QuoteLineItemResponse.model_validate(line_item_dict)
    ...
    def _weekly_hours_to_response(self, weekly_hours: QuoteWeeklyHours) -> QuoteWeeklyHoursResponse:
        """Convert weekly hours model to response schema."""
        return QuoteWeeklyHoursResponse.model_validate({
            "id": weekly_hours.id,
            "week_start_date": weekly_hours.week_start_date,
            "hours": weekly_hours.hours,
        })
    
    def _phase_to_response(self, phase) -> QuotePhaseResponse:
        """Convert QuotePhase model to QuotePhaseResponse."""
        from app.models.quote import QuotePhase
        return QuotePhaseResponse.model_validate({
            "id": phase.id,
