...
import { useUpdateExpenseApprovers } from "@/hooks/useEngagements";
...
interface EngagementExpenseApproversProps {
  engagement: EngagementDetailResponse;
  onRefetch?: () => Promise<unknown>;
}

export function EngagementExpenseApprovers({
  engagement,
  onRefetch,
}: EngagementExpenseApproversProps) {
  const { data: employeesData, isLoading: employeesLoading } = useEmployees({ limit: 1000 });
  const updateApprovers = useUpdateExpenseApprovers({
    onSuccess: async () => {
  ...
  const approvers = engagement.expense_approvers || [];
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={handleAdd} className="space-y-2 p-3 border rounded">
        <div>
          <Label htmlFor="expense_approver_select">Add Expense Approver</Label>
          <div className="flex gap-2">
            <Select
              id="expense_approver_select"
      ...
      <div>
        <Label>Expense Approvers ({approvers.length})</Label>
        {approvers.length > 0 ? (
          ...
          <p className="text-sm text-gray-500 mt-2">
            No expense approvers assigned. Add approvers above.
          </p>
        )}
      </div>
    </div>
