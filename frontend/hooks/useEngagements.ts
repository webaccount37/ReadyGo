 ...
 */
export function useLinkRolesToEngagement(
  options?: UseMutationOptions<void, Error, { engagementId: string; roleIds: string[] }>
) {
  const queryClient = useQueryClient();

  return useMutation<void, Error, { engagementId: string; roleIds: string[] }>({
    mutationFn: ({ engagementId, roleIds }) =>
      engagementsApi.linkRolesToEngagement(engagementId, roleIds),
    onSuccess: () => {
 ...
 */
export function useUnlinkRolesFromEngagement(
  options?: UseMutationOptions<void, Error, { engagementId: string; roleIds: string[] }>
) {
  const queryClient = useQueryClient();
  ...
  return useMutation<void, Error, { engagementId: string; roleIds: string[] }>({
    mutationFn: ({ engagementId, roleIds }) =>
      engagementsApi.unlinkRolesFromEngagement(engagementId, roleIds),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEYS.all });
    },
    ...options,
